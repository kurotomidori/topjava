...
import static ru.javawebinar.topjava.web.ErrorTestData.ERROR_INFO_MATCHER;
import static ru.javawebinar.topjava.web.ErrorTestData.sameDateTimeError;
...
class MealRestControllerTest extends AbstractControllerTest {
    ...
    void invalidUpdate() throws Exception {
        Meal updated = getInvalidUpdated();
        perform(MockMvcRequestBuilders.put(REST_URL + MEAL1_ID).contentType(MediaType.APPLICATION_JSON)
                .with(userHttpBasic(user))
                .content(JsonUtil.writeValue(updated)))
                .andExpect(status().isUnprocessableEntity());
    ...
    @Test
    void createWithInvalidParameters() throws Exception {
    ...
    @Test
    void saveWithSameDateTime() throws Exception {
        MvcResult result = perform(MockMvcRequestBuilders.put(REST_URL + MEAL1_ID).contentType(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                .with(userHttpBasic(user))
                .content(JsonUtil.writeValue(getNewWithSameDateTime())))
                .andExpect(status().isConflict())
                .andReturn();
        ErrorInfo error = JsonUtil.readValue(result.getResponse().getContentAsString(), ErrorInfo.class);
        ERROR_INFO_MATCHER.assertMatch(error, sameDateTimeError);
    }
